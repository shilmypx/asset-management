...
-- Employee sync: manual today, API-ready for later. Storing this now
-- means the HR integration can be switched on later without a schema
-- change — just flip mode to 'api' and fill in the connection details.
create table hr_sync_settings (
  id uuid primary key default gen_random_uuid(),
  mode text not null default 'manual' check (mode in ('manual', 'api')),
  api_endpoint text,
  api_key_encrypted text, -- never store plaintext; the UI only ever shows a masked value once set
  sync_frequency text default 'manual' check (sync_frequency in ('manual', 'hourly', 'daily', 'weekly')),
  last_synced_at timestamptz,
  is_active boolean not null default false
);

-- Label/barcode print layout — the configurable version of what the
-- Barcode Printing screen currently hardcodes.
create table label_print_settings (
  id uuid primary key default gen_random_uuid(),
  name text not null default 'Default',
  is_default boolean not null default true,
  printer_name text,
  page_width_mm numeric(6,2) not null default 210,   -- A4 default
  ...
  font_size_pt numeric(5,2) default 8,
  barcode_format text default 'code128' check (barcode_format in ('code128', 'qr'))
);

-- Who gets notified for which event — the routing layer on top of
-- notification_templates (which holds the message content; this holds
-- the "send it to whom" decision).
create table notification_routing (
  id uuid primary key default gen_random_uuid(),
  event_type text not null, -- matches notification_templates.event_type and/or automation_rules.trigger_event
  recipient_type text not null check (recipient_type in ('specific_email', 'role', 'requester_manager')),
  recipient_email text,     -- used when recipient_type = 'specific_email'
  recipient_role_id uuid references roles(id), -- used when recipient_type = 'role'
  channel text not null default 'email' check (channel in ('email', 'dashboard', 'both')),
  is_active boolean not null default true
);

-- Whether a given request type needs approval, and who the approver is.
-- The approver is always notified via notification_routing's
-- 'approval_requests' event when a rule here has requires_approval = true
-- — that's enforced in the app layer (the approval screens check this
-- table before allowing auto-fulfillment), not re-declared here.
...
insert into approval_rules (request_type, requires_approval) values
  ('purchase_order', true), ('self_service_request', true),
  ('asset_disposal', true), ('change_request', true);

insert into notification_routing (event_type, recipient_type, channel) values
  ('renewal', 'role', 'email'), ('warranty_expiry', 'role', 'email'),
  ('maintenance_due', 'role', 'email'), ('repair_delays', 'role', 'email'),
  ('assignment', 'specific_email', 'dashboard'), ('return', 'specific_email', 'dashboard'),
  ('approval_requests', 'role', 'both'), ('audit_due', 'role', 'email'),
  ('contract_expiry', 'role', 'email'), ('low_license_availability', 'role', 'email');
